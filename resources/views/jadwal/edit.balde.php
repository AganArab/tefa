...
<div class="col-md-12">
    <div class="card card-primary">
        ...
        <form action="{{ route('jadwal.update', ['hari' => $jadwal->hari, 'jadwal' => $jadwal->id]) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="card-body">
                <div class="form-group">
                    ...
                    <input type="text" name="jam_ke" class="form-control @error('jam_ke') is-invalid @enderror"
                        value="{{ old('jam_ke', $jadwal->jam_ke) }}" placeholder="Contoh: 1 atau Istirahat 1" required>
                    @error('jam_ke')
                ...
                <div id="aktif-fields">
                    <div class="form-group">
                        <label for="guru_id">Guru (Mengajar di Jadwal Ini)</label>
                        ...
                        <select name="mapel_id" class="form-control @error('mapel_id') is-invalid @enderror">
                            ...
                            @foreach($mapels as $m)
                                <option value="{{ $m->id }}" {{ old('mapel_id', $jadwal->mapel_id) == $m->id ? 'selected' : '' }}>
                                    {{ $m->nama }} (Guru: {{ $m->guruPengajar?->nama ?? 'N/A' }})
                                </option>
                            @endforeach
                        </select>
                        @error('mapel_id')
                    ...
                    <select name="ruangan_id" class="form-control @error('ruangan_id') is-invalid @enderror" required>
                        ...
                        @foreach($ruangans as $r)
                            <option value="{{ $r->id }}" {{ old('ruangan_id', $jadwal->ruangan_id) == $r->id ? 'selected' : '' }}>
                                {{ $r->nama }} (PJ: {{ $r->penanggungJawab?->nama ?? 'N/A' }})
                            </option>
                        @endforeach
                    </select>
                    @error('ruangan_id')
            ...
            <div class="card-footer d-flex justify-content-between">
                <a href="{{ route('jadwal.hari', $jadwal->hari) }}" class="btn btn-secondary">Batal</a>
