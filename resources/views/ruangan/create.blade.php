...
@section('atas', 'Tambah Ruangan')
@section('judul', 'Form Tambah Ruangan')
            ...
            <div class="card-body">
                <div class="form-group">
                    ...
                    <input type="text" name="nama" class="form-control @error('nama') is-invalid @enderror"
                        value="{{ old('nama') }}" placeholder="Contoh: Information Lab - 1" required>
                    @error('nama')
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="kode" class="form-control @error('kode') is-invalid @enderror"
                        value="{{ old('kode') }}" placeholder="Contoh: IL1">
                    @error('kode')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div class="form-group">
                    ...
                    <input type="number" name="kapasitas" class="form-control @error('kapasitas') is-invalid @enderror"
                        value="{{ old('kapasitas') }}" placeholder="Jumlah kursi (opsional)" min="0">
                    @error('kapasitas')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div class="form-group">
                    ...
                    <select name="penanggung_jawab_id" class="form-control @error('penanggung_jawab_id') is-invalid @enderror">
                        ...
                        @foreach($gurus as $g)
                            <option value="{{ $g->id }}" {{ old('penanggung_jawab_id') == $g->id ? 'selected' : '' }}>
                                {{ $g->nama }}
                            </option>
                        @endforeach
                    </select>
